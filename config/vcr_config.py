...
def _patched_vcrhttpresponse_init(self, recorded_response):
    ...
    if isinstance(body, dict) and "string" in body:
        ...
        if (isinstance(value, str) and value.startswith(_PARTS_PREFIX)) or (
            isinstance(value, bytes) and value.startswith(_PARTS_PREFIX.encode("utf-8"))
        ):
...
def _body_matcher(r1, r2):  # noqa: C901
    ...
    if uri1 != uri2:
        if body1 != body2:
            msg = f"Body mismatch for different URIs: {body1!r} != {body2!r}"
    ...
    if uri1.endswith("/LATEST/eval"):
        ...
        if form1.keys() != form2.keys():
            msg = f"Form keys mismatch: {form1.keys()} != {form2.keys()}"
            raise AssertionError(msg)
        ...
        for key in form1:
            ...
            if key == "vars":
                if _normalise_json_string(v1) != _normalise_json_string(v2):
                    msg = f"vars mismatch: {v1!r} != {v2!r}"
                    raise AssertionError(msg)
            elif v1.strip() != v2.strip():
                msg = f"Form field {key!r} mismatch: {v1!r} != {v2!r}"
                raise AssertionError(msg)

        return

    if body1 != body2:
        msg = f"Body mismatch: {body1!r} != {body2!r}"
        raise AssertionError(msg)
